...
class M_ControllerEvents {
    ...
    initialize_Controller(sequential_startup = true) {
        ...
        if (sequential_startup) {
            this.get_Controller()
                .wait(c_controller_1.C_Controller.AllServices, c_controller_1.C_StartupTalk.run_Listen, undefined, () => {
            ...
            this.get_Controller()
                .wait(c_controller_1.C_Controller.AllServices, c_controller_1.C_StartupTalk.run_Talk, undefined, () => {
                this.register_Announcements();
                this.register_Services();
                this.announce_ToAllServices(c_controller_1.C_BootState.TalkReady);
            }, t_controller_1.e_Scope.Global);
            this.announce_ToAllServices(c_controller_1.C_BootState.ClassReady, 200);
        }
        else {
            this.register_Dependencies();
            this.register_Subscriptions();
    ...
    register_Services() {
        if (this._services) {
            this._services.forEach((service) => {
                this.get_Controller().respond(service.Call, service.Static || false, service.Scope, t_controller_1.e_ServiceGroup.Standard);
            });
        }
    }
    ...
    produce_StepsPromise(scope, manager_namespace, step_promise_stack, step, index) {
        ...
        return step_promise_stack.sniff([index_str], () => {
            throw new Error(c_controller_1.C_Controller.E_ActiveStepMemberCount
                .subs(index_str));
        }, () => {
    ...
    announce_ToAllServices(resolution_instruction, delay = 0) {
        this.get_Controller().announce(c_controller_1.C_Controller.AllServices, resolution_instruction, t_controller_1.e_Scope.Global, delay);
    }
    announce_LibraryAdded(library_source_namespace) {
        this.get_Controller().announce(c_controller_1.C_Controller.AllServices, [...c_controller_1.C_BootState.LibraryAdded, [library_source_namespace]], t_controller_1.e_Scope.Global, true);
    }
}
