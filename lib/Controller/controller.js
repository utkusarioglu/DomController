...
class Controller extends separator_handler_1.SeparatorHandler {
    ...
    respond(response_callback, is_static = true, scope = t_controller_1.e_Scope.Global, group = t_controller_1.e_ServiceGroup.Standard) {
        if (is_static) {
    ...
    wait(recipient_namespace, listen, test_callback = () => true, action_callback = (transmission) => transmission, scope = t_controller_1.e_Scope.Global, count = 1, current_count = count) {
