...
test("Controller.listen&talk.Global", () => {
    ...
    const listen = new Promise((resolve) => {
        c.subscribe(t_controller_1.e_Scope.Global, subscribed_namespace, c_controller_1.C_StartupTalk.send_Archive, (transmission) => {
            resolve(transmission.Talk[2][0]);
        });
    });
    c.announce(t_controller_1.e_Scope.Global, subscribed_namespace, [...c_controller_1.C_StartupTalk.send_Archive, [data]]);
...
test("Controller.service.global", () => {
    ...
    service_controller.respond(t_controller_1.e_Scope.Global, (transmission) => {
        return Promise.resolve(response_data);
    });
    const response = consuming_controller.request(t_controller_1.e_Scope.Global, service_namespace, ["RI", "do_Something"]).
        then((transmission) => {
